...
//               >
//                 {darkMode ? <FaSun className="text-yellow-300" /> : <FaMoon className={classes.iconColor} />}
//               </button>
...
//                 } rounded-2xl p-4 shadow-sm ${message.sender === 'user' ? 'rounded-tr-none' : 'rounded-tl-none'} transition-colors duration-300`}>
...
//                       className={`px-3 py-2 rounded-full text-sm transition-all flex items-center ${
...
//                       }`}
//                     >
//                       <span className="mr-1">{topic.icon}</span> {topic.title}
//                     </button>
//                   ))}
//                 </div>
//               </div>
...
//                   className={`flex-1 max-h-[300px] py-3 resize-none px-3 bg-transparent outline-none ${classes.textColor} placeholder-gray-500 transition-colors duration-300`}
//                 />
//                 <button type="button" className={`p-3 ${classes.iconColor} hover:${classes.accentText}`}>
//                   <FaRegSmile />
//                 </button>
...
//                     {capability.icon}
//                   </span>
//                   <span className='mx-2'>{t(capability.key, capability.text)}</span>
//                 </li>
//               ))}
//             </ul>
//           </div>
...
function ChatPage() {
  ...
  const [messages, setMessages] = useState([
    {
      id: 1,
      sender: 'bot',
      text: t('bot.text', "🚀 Welcome to your cosmic financial journey! I'm Nova, your AI-powered financial guide. Let's explore the universe of wealth together!"),
      timestamp: new Date()
    }
  ]);
  const [inputValue, setInputValue] = useState('');
  ...
  const [activeTopic, setActiveTopic] = useState(null);
  const [darkMode, setDarkMode] = useState(false);
  const [userName, setUserName] = useState('Space Explorer');
  const [showConfetti, setShowConfetti] = useState(false);
  const [currentTheme, setCurrentTheme] = useState('nebula');
  
  const themes = {
    nebula: { 
      primary: 'from-purple-600 via-pink-500 to-red-500',
      secondary: 'to-purple-300',
    ...
    galaxy: { 
      ...
      secondary: 'to-blue-300',
      text: 'text-blue-200',
      bg: 'bg-blue-500/20',
      border: 'border-blue-400/30'
    },
    supernova: { 
      primary: 'from-orange-500 via-yellow-500 to-red-500',
      secondary: 'to-orange-300',
  ...
  const suggestedTopics = [
    { id: 1, title: "Quantum Investing", icon: <FaRocket />, desc: "Explore futuristic investment strategies" },
    { id: 2, title: "Black Hole Budgeting", icon: <FaChartLine />, desc: "Master the gravity of your finances" },
    { id: 3, title: "Stellar Savings", icon: <FaCoins />, desc: "Harness cosmic compound interest" },
    { id: 4, title: "AI Wealth Architect", icon: <FaMagic />, desc: "Custom portfolio construction" },
  ];

  const cosmicMessages = [
    "💫 Did you know? Investing $100 monthly at 8% return becomes $150,000 in 30 years!",
    "🌌 Pro Tip: Automate your savings to build wealth on autopilot",
    "🚀 Fun Fact: The stock market has returned ~10% annually since 1926",
  ];
  ...
  const handleSubmit = (e) => {
    ...
    if(inputValue.toLowerCase().includes('thank')) {
  ...
  const generateResponse = (query) => {
    const lowerQuery = query.toLowerCase();
    const randomCosmicMessage = cosmicMessages[Math.floor(Math.random() * cosmicMessages.length)];
    
    if (lowerQuery.includes('invest')) return `${randomCosmicMessage}\n\n🚀 Let's create your personalized investment constellation!`;
    if (lowerQuery.includes('save')) return `🌠 Excellent choice! Let's supercharge your savings with these black hole strategies: ...`;
    return `🪐 Fascinating question! ${randomCosmicMessage}\n\nLet me analyze the financial cosmos for you...`;
  };
  ...
  const MessageBubble = ({ message }) => (
    <motion.div
      ...
      className={`relative max-w-xl p-6 rounded-3xl ${
        message.sender === 'user' 
          ? 'ml-auto bg-gradient-to-br from-purple-600 to-blue-500 text-white'
          : `dark:bg-gray-800/80 bg-white ${darkMode ? themes[currentTheme].border : 'border-gray-200'} border`
      }`}
    >
      <div className="space-y-2">
        <div className="flex items-center gap-3 text-sm">
          {message.sender === 'bot' && <FaRobot className="text-purple-400" />}
          <span className="font-semibold">
            {message.sender === 'user' ? userName : 'Nova AI'}
          </span>
      ...
      {message.sender === 'bot' && (
        <div className={`absolute -left-3 top-4 w-6 h-6 rotate-45 ${darkMode ? 'bg-gray-800/80' : 'bg-white'} border-l border-t`} />
      )}
    </motion.div>
  );

  return (
    <div className={`min-h-screen ${darkMode ? 'bg-galaxy-dark text-starry-light' : 'bg-starry-light text-galaxy-dark'} transition-colors`}>
      <NavBarbg isLightBackground={!darkMode} />
      
      <div className="flex h-[calc(100vh-4rem)] pt-16">
        {/* Cosmic Sidebar */}
        <div className={`hidden md:block w-80 border-r ${darkMode ? 'border-space-dark' : 'border-starry-light'} p-4 space-y-6`}>
          <button 
            onClick={() => window.location.reload()}
            className="w-full bg-gradient-to-r from-purple-600 to-blue-500 text-white p-3 rounded-2xl font-bold hover:scale-[1.02] transition-transform"
          >
          ...
          <div className="space-y-4">
            <h3 className="text-sm font-bold opacity-70">Star Topics</h3>
            {suggestedTopics.map(topic => (
              <motion.div 
                key={topic.id}
                whileHover={{ scale: 1.02 }}
                className={`p-4 rounded-xl ${darkMode ? 'bg-space-dark hover:bg-space-darker' : 'bg-starry-light hover:bg-gray-50'} cursor-pointer transition-colors`}
                onClick={() => setInputValue(topic.title + '?')}
              >
                <div className="text-purple-400 text-xl mb-2">{topic.icon}</div>
                <div className="font-semibold mb-1">{topic.title}</div>
                <div className="text-sm opacity-70">{topic.desc}</div>
              </motion.div>
            ))}
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col">
          <div className={`p-4 border-b ${darkMode ? 'border-space-dark' : 'border-starry-light'} flex items-center justify-between`}>
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 text-sm text-green-400">
                  ...
                  Interstellar Connection Active
                </div>
              </div>
            </div>
            <div className="flex gap-4">
              <button 
                ...
                className="p-3 rounded-xl bg-opacity-20 hover:bg-opacity-30 transition-all"
              >
                {darkMode ? <FaRegStar className="text-2xl text-yellow-300" /> : <FaPalette className="text-2xl text-purple-600" />}
              </button>
            </div>
          </div>
          ...
          <div 
            ref={chatContainerRef}
            className="flex-1 overflow-y-auto p-6 space-y-8 bg-gradient-to-b from-space-dark/5 to-transparent"
          >
            <AnimatePresence>
              {messages.map(message => (
            ...
            {isTyping && (
              ...
              >
                <div className="p-4 rounded-2xl bg-space-dark">
                ...
                <div className="space-y-2">
                  ...
                  <div className="text-sm opacity-70">Analyzing financial cosmos...</div>
                </div>
              </motion.div>
            )}
          </div>
          ...
          <div className="p-6 pt-0">
            <form onSubmit={handleSubmit} className="relative">
              <div className={`p-1 rounded-2xl backdrop-blur-lg ${darkMode ? 'bg-space-dark/30 border-space-dark' : 'bg-white border-gray-200'} border`}>
                <div className="flex items-center gap-2 p-2">
                  <button type="button" className="p-3 text-purple-400 hover:text-purple-300 rounded-xl">
                    <FaPaperclip />
                  </button>
                  <textarea
                    value={inputValue}
                    onChange={(e) => setInputValue(e.target.value)}
                    placeholder="Transmit your financial query..."
                    className="flex-1 bg-transparent outline-none resize-none p-2 max-h-48"
                  />
                  <button 
                    type="submit"
                    disabled={!inputValue}
                    className={`p-3 rounded-xl ${!inputValue ? 'opacity-50' : 'hover:bg-purple-500/20'} transition-all`}
                  >
                    <FaPaperPlane className="text-xl text-purple-400" />
                  </button>
                </div>
              </div>
            </form>
        ...
        <Confetti active={showConfetti} config={{ elementCount: 200, spread: 180 }} />
      </div>
    </div>
  );
}
